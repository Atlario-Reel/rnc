...
export const updateBookmark = async ({newBookmarkedBy, videoId}) => {
 console.log(videoId, newBookmarkedBy)
  try {
      const updatedBookmark = await databases.updateDocument(
      databaseId,
      videoCollectionId,
      videoId, {
        bookmarkers: newBookmarkedBy
      }
    )
...
export const showBookmarkedPosts = async (query, userId) => {
  if (query.length === 0) {
  try {
    ...
    videoCollectionId,
    [
     Query.contains('bookmarkers', userId) 
    ]
  )
